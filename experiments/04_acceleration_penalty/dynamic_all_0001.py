...
OUTPUT_FILE = "distilgpt2_from_scratch_wikitext103_all_layers_0001.txt"
LAMBDA_ACCEL = 0.0001
BLOCK_SIZE = 128
NUM_TRAIN_EPOCHS = 2
BATCH_SIZE = 16
...
log_message("Penalty applied to ALL layers.")
log_message("-" * 50)
...
class DynamicPenaltyTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        ...
        if self.lambda_coeff > 0 and outputs.hidden_states is not None:
            # 💡 Key Change: Loop over all transformer layer outputs
            # The first hidden state (index 0) is the input embedding, so we skip it.
            transformer_layer_outputs = outputs.hidden_states[1:]
            num_layers_penalized = 0

            for hidden_states_for_layer in transformer_layer_outputs:
                ...
                num_layers_penalized += 1
            
            # Average the penalty over the number of layers to keep the scale consistent
            if num_layers_penalized > 0:
                total_acceleration_penalty /= num_layers_penalized

        total_loss = original_loss + self.lambda_coeff * total_acceleration_penalty
        self.log({"loss_original": original_loss.item(), "loss_acceleration_avg": total_acceleration_penalty.item()})
        return (total_loss, outputs) if return_outputs else total_loss


# --- Section 3: Main Training and Evaluation Function ---
def train_and_evaluate():
    model_type = "distilgpt2_from_scratch_all_layers_0001"
...
def generate_and_log_samples(model_path, tokenizer_obj, num_samples=3):
    ...
    for i in range(num_samples):
        ...
        # We need to create the attention mask manually here as well
...
if __name__ == "__main__":
    ...
    log_message("="*50)
    log_message(f"Model: {MODEL_NAME} (from scratch on wikitext-103, all layers penalized)")
