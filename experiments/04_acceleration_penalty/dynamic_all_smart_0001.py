...
OUTPUT_FILE = "distilgpt2_from_scratch_wikitext103_weighted_all_smart_0001.txt"
LAMBDA_ACCEL = 0.0001
BLOCK_SIZE = 128
NUM_TRAIN_EPOCHS = 2
BATCH_SIZE = 16
...
log_message("Penalty applied to ALL layers with custom weights.")
log_message("-" * 50)
...
class DynamicPenaltyTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        ...
        if self.lambda_coeff > 0 and outputs.hidden_states is not None:
            ...
            num_layers = len(transformer_layer_outputs)

            # 💡 Key Change: Define weights for each layer
            # We give more weight to early/middle layers, which are more crucial for syntax.
            # This example is for a 6-layer model like distilgpt2.
            if num_layers == 6:
                layer_weights = torch.tensor([1.5, 1.3, 1.1, 1.0, 0.8, 0.6], device=model.device)
            else: # Fallback for other models to prevent errors
                layer_weights = torch.ones(num_layers, device=model.device)

            total_weighted_penalty = torch.tensor(0.0, device=model.device)
            
            # 💡 Key Change: Use enumerate to get the layer index
            for i, hidden_states_for_layer in enumerate(transformer_layer_outputs):
                ...
                # 💡 Key Change: Apply the weight for the current layer
                total_weighted_penalty += layer_weights[i] * layer_penalty
            
            # Normalize by the sum of weights to get a weighted average.
            # This keeps the penalty's scale consistent, making lambda easier to tune.
            if layer_weights.sum() > 0:
                total_acceleration_penalty = total_weighted_penalty / layer_weights.sum()

        total_loss = original_loss + self.lambda_coeff * total_acceleration_penalty
        self.log({"loss_original": original_loss.item(), "loss_acceleration_weighted_avg": total_acceleration_penalty.item()})
        return (total_loss, outputs) if return_outputs else total_loss


# --- Section 3: Main Training and Evaluation Function ---
def train_and_evaluate():
    model_type = "distilgpt2_from_scratch_weighted_all_smart_0001"
    log_message(f"\n===== Starting Training for Model: {model_type} =====")
    model = load_model_from_scratch()
    output_dir = f"/storage/ranjbar/dynamicModles/results_{model_type}"
    ...
    final_model_path = f"/storage/ranjbar/dynamicModles/final_model_{model_type}"
...
def generate_and_log_samples(model_path, tokenizer_obj, num_samples=3):
    ...
    for i in range(num_samples):
        log_message(f"\n--- Sample {i+1} ---")
        sample_data = validation_dataset[i * 10]
...
if __name__ == "__main__":
    ...
    log_message("="*50)
    log_message(f"Model: {MODEL_NAME} (from scratch on wikitext-103, weighted layers penalized)")
